...
def align_images(image, template, maxFeatures=500, keepPercent=0.2, debug=True):
    ...
    if debug:
        ...
        cv2.imshow("Matched Keypoint", matchedVis)
        cv2.waitKey(0)


    # allocate memory for the keypoints (x, y)-coordinates from the top matches -- we'll use these coordinates to compute our homography matrix
